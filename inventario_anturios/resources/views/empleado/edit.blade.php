...
<div class="container">
    ...
    <form action="{{ route('empleado.update', $empleado->nro_identificacion) }}" method="POST" class="row g-3">
        ...
        <div class="col-md-6">
            ...
            <input type="text" name="nombreemp" id="nombreemp" class="form-control" value="{{ old('nombreemp', $empleado->nombreemp) }}" required>
        </div>

        <div class="col-md-6">
            <label for="apellidoemp" class="form-label">Apellido</label>
            <input type="text" name="apellidoemp" id="apellidoemp" class="form-control" value="{{ old('apellidoemp', $empleado->apellidoemp) }}" required>
        </div>
        ...
        <div class="col-md-6">
            ...
            <input type="text" name="nro_identificacion" id="nro_identificacion" class="form-control" value="{{ old('nro_identificacion', $empleado->nro_identificacion) }}" required>
        </div>
        ...
        <div class="col-md-6">
            ...
            <select name="idbodega" id="idbodega" class="form-select" required>
                ...
                @foreach ($bodegas as $bodega)
                    <option value="{{ $bodega->idbodega }}" {{ old('idbodega', $empleado->idbodega) == $bodega->idbodega ? 'selected' : '' }}>{{ $bodega->nombrebodega }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="col-md-6">
            ...
            <input type="text" name="nro_telefono" id="nro_telefono" class="form-control" value="{{ old('nro_telefono', $empleado->nro_telefono) }}" required>
        </div>

        <div class="col-md-6">
            <label for="direccionemp" class="form-label">Dirección</label>
            <input type="text" name="direccionemp" id="direccionemp" class="form-control" value="{{ old('direccionemp', $empleado->direccionemp) }}" required>
        </div>
